import { useState, useEffect } from 'react';
import { Calendar, MapPin, Users, CheckCircle, Ticket, Loader2 } from 'lucide-react';
import confetti from 'canvas-confetti';
import { api } from '@/lib/apiClient';

// Fallback images/tags since DB might only have basic text
const MOCK_IMAGES = [
  "https://images.unsplash.com/photo-1618477461853-cf6ed80faba5?auto=format&fit=crop&q=80&w=600",
  "https://images.unsplash.com/photo-1550989460-0adf9ea622e2?auto=format&fit=crop&q=80&w=600",
  "https://images.unsplash.com/photo-1595278069441-2cf29f8005a4?auto=format&fit=crop&q=80&w=600"
];

export const EventsTab = () => {
  const [events, setEvents] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [joiningId, setJoiningId] = useState<number | null>(null);
  const [joinedIds, setJoinedIds] = useState<number[]>([]); // Track locally for session

  // 1. Fetch Events on Mount
  useEffect(() => {
    const loadEvents = async () => {
      try {
        const data = await api.getCitizenEvents();
        setEvents(data);
      } catch (err) {
        console.error("Failed to load events", err);
      } finally {
        setLoading(false);
      }
    };
    loadEvents();
  }, []);

  // 2. Handle Join Logic
  const handleJoin = async (id: number) => {
    setJoiningId(id);
    try {
      const res = await api.joinEvent(String(id)); // Backend expects String ID
      
      // Success!
      if (res.success) {
         setJoinedIds(prev => [...prev, id]);
         confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 } });
         
         // Update attendee count locally
         setEvents(prev => prev.map(ev => 
            ev.id === id ? { ...ev, registrations: (ev.registrations || 0) + 1 } : ev
         ));
         
         alert(`Success! You earned points. Check your dashboard.`);
      }
    } catch (err: any) {
      if (err.message.includes("already joined")) {
          setJoinedIds(prev => [...prev, id]); // Sync local state
          alert("You are already registered for this event!");
      } else {
          alert("Failed to join event. Please try again.");
      }
    } finally {
      setJoiningId(null);
    }
  };

  if (loading) {
     return <div className="h-64 flex items-center justify-center"><Loader2 className="animate-spin w-10 h-10 text-brand-neon" /></div>;
  }

  return (
    <div className="pb-24 space-y-6">
      
      {/* HEADER */}
      <div className="bg-green-600 text-white p-8 rounded-[32px] border-2 border-green-800 shadow-neo relative overflow-hidden">
         <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-20"></div>
         <div className="relative z-10">
            <h1 className="text-4xl font-black italic uppercase leading-none mb-2">Squad Missions</h1>
            <p className="font-bold text-green-100">Join forces. Clean Bhopal. Earn massive rep.</p>
         </div>
      </div>

      {/* EVENT CARDS */}
      <div className="space-y-6">
        {events.length === 0 ? (
            <div className="text-center p-10 bg-gray-100 rounded-3xl dark:bg-gray-800">
                <p className="font-bold text-gray-500">No active missions right now.</p>
            </div>
        ) : (
            events.map((event, index) => {
               // Determine visual state
               const isJoined = joinedIds.includes(event.id);
               const image = event.image_url || MOCK_IMAGES[index % MOCK_IMAGES.length];
               // If DB doesn't have points column, default to 500
               const points = event.points || 500; 

               return (
                 <div key={event.id} className="bg-white dark:bg-gray-900 border-2 border-black dark:border-gray-700 rounded-[32px] overflow-hidden shadow-neo group hover:-translate-y-1 transition-transform">
                   
                   {/* Image Header */}
                   <div className="h-48 overflow-hidden relative">
                      <img src={image} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />
                      
                      <div className="absolute top-4 left-4 flex gap-2">
                         <span className="bg-black text-white px-3 py-1 rounded-full text-[10px] font-black uppercase">Community</span>
                         <span className="bg-green-500 text-black px-3 py-1 rounded-full text-[10px] font-black uppercase">Active</span>
                      </div>

                      <div className="absolute bottom-4 right-4 bg-brand-neon text-black px-4 py-2 rounded-xl border-2 border-black font-black text-sm shadow-[4px_4px_0px_black]">
                         +{points} PTS
                      </div>
                   </div>

                   {/* Content */}
                   <div className="p-6">
                      <div className="flex justify-between items-start mb-4">
                         <div>
                            <h3 className="text-2xl font-black italic uppercase dark:text-white leading-none mb-2">{event.title}</h3>
                            <div className="flex flex-col gap-1 text-sm font-bold text-gray-500">
                               <span className="flex items-center gap-2">
                                  <Calendar size={16}/> {new Date(event.date).toLocaleDateString()}
                               </span>
                               <span className="flex items-center gap-2">
                                  <MapPin size={16}/> {event.location}
                               </span>
                            </div>
                         </div>
                         <div className="text-center bg-gray-100 dark:bg-gray-800 p-2 rounded-xl">
                            <Users size={20} className="mx-auto mb-1 text-gray-500"/>
                            <span className="font-black text-lg block leading-none dark:text-white">{event.registrations || 0}</span>
                            <span className="text-[10px] uppercase font-bold text-gray-400">Going</span>
                         </div>
                      </div>

                      {/* Action Button */}
                      {isJoined ? (
                         <button disabled className="w-full py-4 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 border-2 border-green-200 dark:border-green-800 rounded-xl font-black uppercase flex items-center justify-center gap-2">
                            <Ticket size={20} /> You're Going!
                         </button>
                      ) : (
                         <button 
                            onClick={() => handleJoin(event.id)} 
                            disabled={joiningId === event.id}
                            className="w-full py-4 bg-black text-white dark:bg-white dark:text-black rounded-xl font-black uppercase flex items-center justify-center gap-2 hover:opacity-80 transition-opacity disabled:opacity-50"
                         >
                            {joiningId === event.id ? <Loader2 className="animate-spin" /> : <>RSVP Now <CheckCircle size={20} /></>}
                         </button>
                      )}
                   </div>
                 </div>
               );
            })
        )}
      </div>
    </div>
  );
};